...
func main() {
	pulumi.Run(func(ctx *pulumi.Context) error {
		...
		// Configurable AMI name, with default to Ubuntu 22.04
		amiName := cfg.Get("amiName")
		if amiName == "" {
			amiName = "ubuntu/images/hvm-ssd/ubuntu-jammy-22.04-amd64-server-*"
		}

		// Look up the latest Ubuntu 22.04 AMI or any other configured AMI.
		ami, err := ec2.LookupAmi(ctx, &ec2.LookupAmiArgs{
			Filters: []ec2.GetAmiFilter{
				{
					Name: "name",
					Values: []string{
						amiName, // Ubuntu 22.04 by default
					},
				},
			},
			Owners: []string{
				"099720109477", // Canonical (the owner of Ubuntu images)
			},
		...
		server, err := ec2.NewInstance(ctx, "server", &ec2.InstanceArgs{
			...
			Tags: pulumi.StringMap{
				"Name": pulumi.String("cluster-node1"),
			},
		})
		if err != nil {
			return err
