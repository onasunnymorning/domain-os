	...
	registry := []WorkflowMeta{
		{
			...
			Steps: []WorkflowStep{
				...
				{Key: "validate-registrant-refs", Label: "Validate Contact References", ActivityName: "ValidateRegistrantRefs"},
		...
		},
		{
			...
			Steps: []WorkflowStep{
				...
				{Key: "batch-purge", Label: "Batch Purge", ActivityName: "BatchPurgeDomains"},
			},
		...
		},
		{
			...
			Steps: []WorkflowStep{
				...
				{Key: "batch-restore", Label: "Batch Restore", ActivityName: "BatchRestoreDomains"},
			},
		...
		{
			Key:         "tombstone-backfill",
			Name:        "Tombstone Backfill",
			Description: "Backfills tombstone records for historically purged domains by scanning domain.purged events",
			Queue:       temporal.QueueLifecycle,
			Category:    "operations",
			Tags:        []string{"operations", "tombstone", "backfill", "domain", "archival"},
			Steps: []WorkflowStep{
				{Key: "count-purge-events", Label: "Count Purge Events", ActivityName: "CountPurgeEvents"},
				{Key: "backfill-batch", Label: "Backfill Batch", ActivityName: "BackfillTombstonesBatch"},
				{Key: "count-remaining", Label: "Count Remaining", ActivityName: "CountPurgeEventsWithoutTombstones"},
			},
			docFile: "tombstoneBackfill.doc.md",
		},
		{
			Key:         "serial-drift",
			Name:        "Serial Drift Monitor",
			Description: "Queries master and slave nameservers for SOA serials, evaluates drift, and alerts on stalled propagation",
			Queue:       temporal.QueueFastOps,
			Category:    "operations",
			Tags:        []string{"operations", "dns", "serial", "drift", "monitoring", "GO"},
			Steps: []WorkflowStep{
				{Key: "get-slaving-config", Label: "Get Slaving Config", ActivityName: "GetSlavingConfig"},
				{Key: "get-recent-history", Label: "Get Recent History", ActivityName: "GetRecentHistory"},
				{Key: "fan-out-dns-queries", Label: "Fan-Out DNS Queries", ActivityName: "QuerySOASerial"},
				{Key: "evaluate-drift", Label: "Evaluate Drift"},
				{Key: "persist-observations", Label: "Persist Observations", ActivityName: "PersistObservations"},
				{Key: "raise-alert", Label: "Raise Alert", ActivityName: "RaiseAlert"},
			},
			docFile: "serialDrift.doc.md",
		},
	}
