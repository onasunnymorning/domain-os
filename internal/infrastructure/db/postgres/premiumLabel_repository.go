...
func (plr *PremiumLabelRepository) List(ctx context.Context, pagesize int, cursor string) ([]*entities.PremiumLabel, error) {
	dbpls := []*PremiumLabel{}

	err := plr.db.WithContext(ctx).Order("label ASC").Limit(pagesize).Find(&dbpls, "label > ?", cursor).Error
	if err != nil {
		return nil, err
	}
