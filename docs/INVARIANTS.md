...
**Enforcement status.** Five rules are now enforced in CI by the `arch-lint` job, which runs [`.golangci.arch.yml`](../.golangci.arch.yml) and fails the build: `INV-01`, `INV-03`, `INV-12` (workflows) and `INV-14` (with two named exceptions) via `depguard`, plus `INV-06` as an architecture test. The gate is currently green.
...
The architectural rules were decoupled from that backlog rather than waiting behind it: `.golangci.arch.yml` is a separate config running only the rules that hold today, in its own blocking job. Any lint-based rule that is currently green can be added there immediately. `INV-07` (`errorlint`) and `INV-15` (`staticcheck` SA1029) are the obvious next candidates — SA1029 already reports 0, and `errorlint` needs its 49 sites cleared first ([#404](https://github.com/onasunnymorning/domain-os/issues/404)).
