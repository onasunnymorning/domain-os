...
| INV-15 | **ENFORCED** | `staticcheck` SA1029 (`.golangci.arch.yml`) | — | Live in the `arch-lint` job, scoped to SA1029 alone — the rest of `staticcheck` stays advisory in `.golangci.yml`. Landed green with no exceptions. |
...
**Enforcement status.** Seven rules are now enforced in CI by the `arch-lint` job, which runs [`.golangci.arch.yml`](../.golangci.arch.yml) and fails the build: `INV-01`, `INV-03`, `INV-12` (workflows) and `INV-14` (two named exceptions) via `depguard`; `INV-07` via `errorlint` (eight named exceptions); `INV-15` via `staticcheck` SA1029; and `INV-06` as an architecture test. The gate is currently green, and each rule has been verified to fail on an injected violation rather than merely to pass.

**The remaining blocker, and why it no longer gates this table:**

`.github/workflows/ci.yaml` still runs the main `golangci-lint` pass with `--issues-exit-code=0`, so the seven linters in [`.golangci.yml`](../.golangci.yml) remain advisory. Measured 2026-08-25, they report **1255 issues** — `errcheck` 559, `gocritic` 454, `gosec` 151, `staticcheck` 71, `unused` 13, `ineffassign` 7. That is a triage job, not a flag flip: some are real defects, some are `gocritic` style opinions worth reconfiguring, some warrant a documented `nolint`. Tracked in [#411](https://github.com/onasunnymorning/domain-os/issues/411).
