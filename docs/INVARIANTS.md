...
|---|---|---|---|---|
| INV-01 | **ENFORCED** | `depguard` (`.golangci.arch.yml`) | — | Live in the `arch-lint` CI job: `go.opentelemetry.io` and `newrelic` denied from the relay workflow, relay activities and publisher. Cannot mechanically check "telemetry isn't load-bearing" in general; this pins the concrete path. |
...
| INV-03 | **ENFORCED** | `depguard` (`.golangci.arch.yml`) | — | Live in the `arch-lint` CI job: the Anthropic SDK is denied outside `internal/askg/provider/**`. Landed green — 0 violations. |
...
| INV-06 | **ENFORCED** | architecture test | — | [`workflows/determinism_test.go`](../internal/application/workflows/determinism_test.go) parses every file in the package and rejects nondeterministic imports and `time.Now`/`time.Since` calls. Verified to fail on an injected violation, not merely to pass. Test files are checked for nondeterminism but exempted from the IO rules, since Temporal never replays them. |
| INV-07 | **ENFORCED (with 8 exceptions)** | `errorlint` (`.golangci.arch.yml`) | — | Live in the `arch-lint` job. The 8 files still holding the 47-site backlog are allowlisted by path with per-file counts, so the backlog cannot grow anywhere else; delete each line as its file is cleared ([#404](https://github.com/onasunnymorning/domain-os/issues/404)). **`pkg/domain/entities/domain.go` was fixed rather than excluded** — an unwrapped error in the domain layer defeats `errors.Is` for every caller above it, so the domain layer is fully gated. Only the `errorf` check is on; `asserts` and `comparison` add ~38 more and are a separate decision, since `==` → `errors.Is` can change behaviour where a sentinel is compared by identity. |
...
| INV-12 | **ENFORCED (workflows)** | `depguard` (`.golangci.arch.yml`) | — | `log` and `log/slog` denied from `internal/application/workflows/**`, which is clean. **Activities deliberately excluded**: some accept a `*log.Logger` as an injected dependency and some log at construction time where no activity context exists, so a blanket ban there would be wrong. Extending it needs those cases triaged first. |
...
`.github/workflows/ci.yaml` no longer passes `--issues-exit-code=0`, and the `Lint` job fails the build. The backlog it was hiding was larger than the 1255 recorded when this section was written: `issues.uniq-by-line` defaults to true, which keeps only one finding per source line across all linters, and it was concealing a further 344 — including all 293 `gosec` G104s, which shared their lines with the `errcheck` findings they duplicated. The true starting count was **1599**.

About two thirds of that was configuration rather than defects. Narrowing `gocritic` to its `diagnostic` tag, dropping `staticcheck`'s `QF*` refactoring family, de-duplicating `gosec` G104 against `errcheck`, and applying the standard `std-error-handling` exclusions accounted for 1093 findings that were never real. The remaining ~500 were triaged individually. `.golangci.yml` records the reasoning for each rule-level decision inline, including what would make a disabled rule worth re-enabling.

Two conventions came out of that work and are worth keeping: a suppression carries its reason at the site (`//nolint:<linter> // why`, `#nosec Gxxx -- why`), and a rule that genuinely does not fit the codebase is argued once in `.golangci.yml` rather than rubber-stamped at dozens of call sites.

The architectural rules were decoupled from that backlog rather than waiting behind it: `.golangci.arch.yml` is a separate config running only the rules that hold today, in its own blocking job. A rule with a bounded, enumerable backlog does not have to wait either — `INV-07` and `INV-14` are enforced with their known violations allowlisted by path, which stops the backlog growing while the clean-up issues clear it. That is the pattern to reuse: gate the rule, name the exceptions, delete each exception as it is fixed.
...
**All three of the suggested first steps have landed:** INV-06 (architecture test) · INV-03 (one `depguard` block) · removing `--issues-exit-code=0` ([#411](https://github.com/onasunnymorning/domain-os/issues/411)). What remains partial in the table above is the set needing a custom analyser or an architecture test with real semantic content — INV-04, INV-05, INV-08, INV-09, INV-10, INV-11, INV-13, INV-16.
